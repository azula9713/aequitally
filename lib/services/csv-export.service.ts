...
export class SettlementCSVExportService {
	...
	private processSettlementData(
		...
		const balances = tally.participants.map((participant) => {
			...
			const totalOwed = tally.expenses.reduce((sum, exp) => {
				const share = exp.shareBetween.find(
					(s) => s.participantId === participant.userId,
				);
		...
		const totalAmount = tally.expenses.reduce(
			(sum, exp) => sum + exp.amount,
			0,
		);
		const transferCount = transfers.length;
		const largestTransfer =
			transfers.length > 0 ? Math.max(...transfers.map((t) => t.amount)) : 0;
